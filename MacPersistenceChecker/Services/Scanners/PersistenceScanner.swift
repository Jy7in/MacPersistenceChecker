...
final class ScannerOrchestrator: ObservableObject {
    ...
    /// Risk scorer for security assessment
    ...
    init(trustVerifier: TrustVerifier = TrustVerifier(), configuration: ScannerConfiguration = .shared, riskScorer: RiskScorer = RiskScorer()) {
    ...
    func scanAll() async -> [PersistenceItem] {
        ...
        progress = 0.8 // Verification done, now risk scoring

        // PHASE 3: Calculate risk scores
        let scoredItems = await assessRisk(verifiedItems)

        progress = 1.0
        scanStats.totalItems = scoredItems.count
    ...
    private func assessRisk(_ items: [PersistenceItem]) async -> [PersistenceItem] {
        let scorer = riskScorer

        return await Task.detached {
            var results: [PersistenceItem] = []
            let timestampExtractor = FileTimestampExtractor.shared

            for var item in items {
                // Extract file timestamps for forensics
                let timestamps = timestampExtractor.getItemTimestamps(
                    plistPath: item.plistPath,
                    binaryPath: item.effectiveExecutablePath
                )
                item.plistCreatedAt = timestamps.plistCreated
                item.plistModifiedAt = timestamps.plistModified
                item.binaryCreatedAt = timestamps.binaryCreated
                item.binaryModifiedAt = timestamps.binaryModified
                item.binaryLastExecutedAt = timestamps.binaryLastExecuted

                // Extract entitlements if we have an executable
                var entitlements: [String: Any]? = nil
                if let execPath = item.effectiveExecutablePath {
                    entitlements = RiskScorer.extractEntitlements(from: execPath)
                }

                // Calculate risk assessment
                let assessment = scorer.assess(item, entitlements: entitlements)

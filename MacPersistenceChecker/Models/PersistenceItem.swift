...
struct PersistenceItem: Identifiable, Equatable, Codable, Hashable {
    ...
    // MARK: - Launch Frequency (for anomaly detection)

    /// StartInterval - how often the job runs (in seconds)
    var startInterval: Int?

    /// ThrottleInterval - minimum time between launches (in seconds)
    var throttleInterval: Int?

    /// StartCalendarInterval - scheduled run times
    var startCalendarInterval: [[String: Int]]?
